...
function loadState() {
  ...
  try {
    ...
    const savedMediaRoots = normalizeRootList(
      parsed.mediaRoots ?? [],
      "Saved media roots",
    );
    const savedComicRoots = normalizeRootList(
      parsed.comicRoots ?? [],
      "Saved comic roots",
    );
    ...
    console.warn(
      `Failed to read ${ROOTS_FILE_PATH}. Falling back to .env roots: ${error.message}`,
    );
...
function removeMediaRoot(rootPath) {
  ...
  const nextRoots = state.mediaRoots.filter(
    (root) => !pathEquals(root.path, resolvedPath),
  );
...
function removeComicRoot(rootPath) {
  ...
  const resolvedPath = path.resolve(rootPath.trim());
  const nextRoots = state.comicRoots.filter(
    (root) => !pathEquals(root.path, resolvedPath),
  );
