...
import { getMediaType } from "../../utils/media-types.js";

function scanMediaFile(rootName, parentFolder, fullPath, stats, existingFilesMap = null) {
...
function scanDirectory(rootName, directory, parentFolder = null, existingFilesMap = null) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      const nestedFiles = scanDirectory(rootName, fullPath, nextParentFolder, existingFilesMap);
    ...
    try {
      ...
      console.warn(`Could not read file information for ${fullPath}:`, error.message);
      continue;
    }

    const mediaFile = scanMediaFile(rootName, parentFolder, fullPath, stats, existingFilesMap);
